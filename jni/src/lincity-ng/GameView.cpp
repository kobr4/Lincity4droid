...
void GameView::scroll( void )
{
    ...
    if( keyScrollState == 0 && mouseScrollState == 0 ) { 
        if (! (fingerScrollX | fingerScrollY) )
        return;
    }
   ...
   viewport.x += stepx*(float)fingerScrollX * 0.5;
   viewport.y += stepy*(float)fingerScrollY * 0.5;
       //LOGD("[Lincity]GameView::scroll() viewport %f %f\n",fingerScrollX,fingerScrollY);
   fingerScrollX = 0;
   fingerScrollY = 0;
   requestRedraw();
}
...
void GameView::event(const Event& event)
{
    switch(event.type) {
        ...
        //NMY
        case Event::FINGERMOTION: {
          /*
            if( event.fingermove.x > 0 ) {
                fingerScrollX =
            } else if( event.fingermove.x < 0 ) {
                fingerScrollX;
            }
            if( event.fingermove.y > 0 ) {
                mouseScrollState |= SCROLL_UP;
            } else if( event.fingermove.y < 0 ) {
                mouseScrollState |= SCROLL_DOWN;
            }
            */
