...
void initPhysfs(const char* argv0)
{
    ...
    LOGD("[Lincity]initPhysfs() - After PHYSFS_init()\n");
    ...
    sprintf(writedir, "%s%s", userdir, application);
    sprintf(writedir,"/mnt/sdcard/lincity");
    LOGD("[Lincity]initPhysfs() - Before PHYSFS_setWriteDir()\n");
    LOGD("[Lincity]initPhysfs() - application=%s\n",application);
    LOGD("[Lincity]initPhysfs() - userdir=%s\n",userdir);
    LOGD("[Lincity]initPhysfs() - dirsep=%s\n",dirsep);
    LOGD("[Lincity]initPhysfs() - writedir=%s\n",writedir);
    if(!PHYSFS_setWriteDir(writedir)) {
        LOGD("[Lincity]initPhysfs() - After PHYSFS_setWriteDir(): can't write to disk\n");
        // try to create the directory
        char* mkdir = new char[strlen(application) + 2];
        sprintf(mkdir, "%s", application);
        if(!PHYSFS_setWriteDir(userdir) || !PHYSFS_mkdir(mkdir)) {
            LOGD("[Lincity]initPhysfs() - After PHYSFS_setWriteDir(): can't write to disk 3rd\n");
        ...
        if(!PHYSFS_setWriteDir(writedir)) {
            LOGD("[Lincity]initPhysfs() - After PHYSFS_setWriteDir(): can't write to disk 2nd\n");
            std::ostringstream msg;
    ...
    LOGD("[Lincity]initPhysfs() - After PHYSFS_setWriteDir(): OK\n");
...
//do when file in in archive?
    ...
    LOGD("[Lincity]initPhysfs() - After PHYSFS_getBaseDir(): dir=%s\n",dir.c_str());
    dir += "/data";
...
#endif
    //NMY
    if(!PHYSFS_addToSearchPath("/mnt/sdcard/lincity/data", 1)) {

    }
    PHYSFS_addToSearchPath("/mnt/sdcard/lincity/.lincity-ng/", 1);
...
{
    LOGD("[Lincity]initVideo()\n");
    int bpp = 32;
    int flags = 0;
    if( getConfig()->useOpenGL ){
        LOGD("[Lincity]initVideo() - getConfig()->useOpenGL\n");
        flags = SDL_OPENGL;
    ...
    LOGD("[Lincity]initVideo() - Before SDL_SetVideoMode()\n");
    SDL_Surface* screen
    ...
    if(!screen) {
        LOGD("[Lincity]initVideo() - After SDL_SetVideoMode() Couldn't set video mode\n");
        std::stringstream msg;
    ...
    if( getConfig()->useOpenGL ){
        ...
        glLoadIdentity();
        glOrthof((float)0, (float)screen->w, (float)screen->h, (float)0, (float)-1, (float)1);
...
void mainLoop()
{
    ...
    LOGD("[Lincity]mainLoop() - Before game.get()\n");
    ...
    LOGD("[Lincity]mainLoop() - Before initLincity()\n");
    initLincity();
    LOGD("[Lincity]mainLoop() - After initLincity()\n");
...
void parseCommandLine(int argc, char** argv)
{
    ...
    for(int currentArgument = 1; currentArgument < argc; ++currentArgument) {
        ...
        if(argStr == "-v" || argStr == "--version") {
            ...
            LOGD("[Lincity]parseCommandLine() kikoo\n");
            argStr = argv[currentArgument];

            LOGD("[Lincity]parseCommandLine() -S %s\n",argStr.c_str());
...
int main(int argc, char** argv)
{
...
#endif
        initPhysfs(argv[0]);
        LOGD("[Lincity] - After initPhysfs()\n");
        dictionaryManager = new TinyGetText::DictionaryManager();
        dictionaryManager->set_charset("UTF-8");
        dictionaryManager->add_directory("locale");
        LOGD("[Lincity] - After DictionaryManager()\n");
    ...
    parseCommandLine(argc, argv); // Do not use getConfig() before parseCommandLine.
     LOGD("[Lincity] - After parseCommandLine()\n");
...
#endif
        xmlInitParser ();
        LOGD("[Lincity] - After xmlInitParser()\n");
        std::auto_ptr<Sound> sound;
        sound.reset(new Sound());
        initSDL();
        LOGD("[Lincity] - After initSDL()\n");
        initTTF();
        LOGD("[Lincity] - After initTTF()\n");
        LOGD("[Lincity] - Before initVideo() %d %d\n",getConfig()->videoX,getConfig()->videoY);
        initVideo(getConfig()->videoX, getConfig()->videoY);
        LOGD("[Lincity] - After initVideo() %d %d\n",getConfig()->videoX,getConfig()->videoY);
        mainLoop();
        LOGD("[Lincity] - After mainLoop()\n");
#ifndef DEBUG
    } catch(std::exception& e) {
        std::cerr << "Unexpected exception: " << e.what() << "\n";
        result = 1;
