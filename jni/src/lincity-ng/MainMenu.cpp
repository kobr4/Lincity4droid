...
extern int g_android_pause;
extern int g_android_quit;
extern void initVideo(int width, int height);
...
{
    if(mainMenu.get() == 0) {
        LOGD("[Lincity]MainMenu::loadMainMenu() - Before mainMenu.reset()\n");
        mainMenu.reset(loadGUIFile("gui/mainmenu.xml"));
        LOGD("[Lincity]MainMenu::loadMainMenu() - After mainMenu.reset()\n");
        ...
        LOGD("[Lincity]MainMenu::loadMainMenu()\n");
    }
...
void MainMenu::fillLoadMenu( bool save /*= false*/ )
{
    ...
    for(int i=0;i<6;i++) {
        ...
        for(char** i = rc; *i != 0; i++){
            curfile = *i;
            LOGD("[Lincity]MainMenu::fillLoadMenu() FILE: %s\n",curfile);
...
MainMenu::run()
{
    ...
    while(running) {
        ...
        //NMY Pausing
        while (g_android_pause && (!g_android_quit))
        {
              SDL_Delay(100);
              if (g_android_pause == 0)
              {
                //initVideo(1280,752);
              }
        }
